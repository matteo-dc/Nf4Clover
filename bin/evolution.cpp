...
double alphas(int Nf, double mu2, int nloop) // mu2 is in physical units (dimensional quantity)
{
  ...
  beta_2 = (2857. / 54.) * pow(Nc, 3) + pow(CF, 2) * Nf - 205. / 18. * CF * Nc * Nf -
           1415. / 54. * pow(Nc, 2) * Nf + 11. / 9. * CF * pow(Nf, 2) + 79. / 54. * Nc * pow(Nf, 2);

  beta_3 =
      (150653. / 486. - 44. / 9. * Z3) * pow(Nc, 4) +
      (-39143. / 162. + 68. / 3. * Z3) * pow(Nc, 3) * Nf +
      (7073. / 486. - 328. / 9. * Z3) * CF * pow(Nc, 2) * Nf +
      (-2102. / 27. + 176. / 9. * Z3) * pow(CF, 2) * Nc * Nf + 23. * pow(CF, 3) * Nf +
      (3965. / 162. + 56. / 9. * Z3) * pow(Nc, 2) * pow(Nf, 2) +
  ...
  if (nloop == 3)
    als3 = als2 +
           pow(als0, 4) * (pow(b1, 3) * (-pow(LL2, 3) + 5. / 2. * pow(LL2, 2) + 2 * LL2 - 1. / 2.) -
                           3. * b1 * b2 * LL2 + b3 / 2.);
  else
    als3 = als2;
...
double S_evolution_to_RIp(int Nf, double p2, double p2ref)
{
  ...
  {
    cmu =
        pow(alm, -12. / 29) * (1. - 8.55727 * alm - 125.423 * pow(alm, 2) - 3797.71 * pow(alm, 3));

    cmu0 =
        pow(al0, -12. / 29) * (1. - 8.55727 * al0 - 125.423 * pow(al0, 2) - 3797.71 * pow(al0, 3));
  }
  if (Nf == 0)
  {
    cmu = pow(alm, -4. / 11) * (1. - 8.08264 * alm - 151.012 * pow(alm, 2) - 5247.93 * pow(alm, 3));

    cmu0 =
        pow(al0, -4. / 11) * (1. - 8.08264 * al0 - 151.012 * pow(al0, 2) - 5247.93 * pow(al0, 3));
  }
  if (Nf == 4)
  {
    cmu =
        pow(alm, -12. / 25) * (1. - 9.38987 * alm - 96.2883 * pow(alm, 2) - 2403.82 * pow(alm, 3));

    cmu0 =
        pow(al0, -12. / 25) * (1. - 9.38987 * al0 - 96.2883 * pow(al0, 2) - 2403.82 * pow(al0, 3));
  }
...
oper_t oper_t::evolve(const double _ainv, const double p2_evol)
{
  ...
  cout << "   p2min[" << step_imom_min << "]=" << p2[step_imom_min] * pow(_ainv, 2.0) << "  ("
       << stepfunc_min << ")" << endl;
  cout << "   p2max[" << step_imom_max << "]=" << p2[step_imom_max] * pow(_ainv, 2.0) << "  ("
       << stepfunc_max << ")" << endl
       << endl;
  ...
  cq_step = q_evolution_to_RIp(Nf, p2[step_imom_min] * pow(_ainv, 2.0),
                               p2[step_imom_max] * pow(_ainv, 2.0));
  /**/
    ...
    for (int ibil = 0; ibil < 5; ibil++)
      for (int ijack = 0; ijack < njacks; ijack++)
        for (int mr1 = 0; mr1 < _nmr; mr1++)
          ...
          {
            (out.jZ)[ibilmom][ibil][ijack][mr1][mr2] =
                jZ[ibilmom][ibil][ijack][mr1][mr2] / cO[ibil];
          }
  }

  cO_step[0] = S_evolution_to_RIp(Nf, p2[step_imom_min] * pow(_ainv, 2.0),
                                  p2[step_imom_max] * pow(_ainv, 2.0)); // S
  cO_step[1] = 1.0;                                                     // A
  cO_step[2] = P_evolution_to_RIp(Nf, p2[step_imom_min] * pow(_ainv, 2.0),
                                  p2[step_imom_max] * pow(_ainv, 2.0)); // P
  cO_step[3] = 1.0;                                                     // V
  cO_step[4] = T_evolution_to_RIp(Nf, p2[step_imom_min] * pow(_ainv, 2.0),
                                  p2[step_imom_max] * pow(_ainv, 2.0)); // T
  /**/
  ...
  for (int ijack = 0; ijack < njacks; ijack++)
    for (int ibil = 0; ibil < 5; ibil++)
    {
      (out.stepfunc)[ijack][ibil + 1] =
          jZ[step_imom_max][ibil][ijack][0][0] / jZ[step_imom_min][ibil][ijack][0][0];
      (out.stepfunc)[ijack][ibil + 1] -= 1.0 / cO_step[ibil];
    }

  for (int ijack = 0; ijack < njacks; ijack++)
  {
    (out.stepfunc)[ijack][nbil + 1] =
        jZVoverZA[step_imom_max][0][ijack][0][0] / jZVoverZA[step_imom_min][0][ijack][0][0];
    (out.stepfunc)[ijack][nbil + 1] -= 1.0;
    (out.stepfunc)[ijack][nbil + 2] =
        jZPoverZS[step_imom_max][0][ijack][0][0] / jZPoverZS[step_imom_min][0][ijack][0][0];
    (out.stepfunc)[ijack][nbil + 2] -= 1.0;
    (out.stepfunc)[ijack][nbil + 3] =
        jZAoverZV[step_imom_max][0][ijack][0][0] / jZAoverZV[step_imom_min][0][ijack][0][0];
