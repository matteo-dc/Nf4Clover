...
oper_t oper_t::a2p2_extr_with_pole(double _ainv)
{
    cout<<endl;
...
{
    cout<<endl;
    cout<<"----- removing hadr. cont. ~ 1/p^2 -----"<<endl<<endl;
    ...
    for(int ibil=0;ibil<nbil;ibil++)
    {
      if(ibil==1) // V
      {
        ...
        for(int imom=0;imom<_bilmoms;imom++)
          for(int ijack=0;ijack<njacks;ijack++)
          {
            (out.jZ)[imom][ibil][ijack][0][0] -= jZ_pars[ijack][2]/coord[1][imom];
          }
      }
    }
...
//        Zq_EM_p2_data<<(in.p2)[imom]<<"\t"<<Zq_EM_ave[imom][0]<<"\t"<<Zq_EM_err[imom][0]<<endl;
    }
...
//            Zbil_EM_p2_data[ibil]<<(in.p2)[imomk]<<"\t"<<Z_EM_ave[imom][ibil][0][0]<<"\t"<<Z_EM_err[imom][ibil][0][0]<<endl;
        }
    }
