...
voper_t combined_M5_quadratic(voper_t in)  
{
  ...
  for(int ibil=0; ibil<nbil; ibil++)
  {
    ...
    cout<<"  -- d4["<<str_bil[ibil]<<"] = "<<get<0>(ave_err(jd4))<<"+/-"<<get<1>(ave_err(jd4))<<endl;
    cout<<"    ** chisqr["<<str_bil[ibil]<<"] = "<<get<0>(ave_err(jchisq))<<"+/-"<<get<1>(ave_err(jchisq))<<endl<<endl;
  }
...
void oper_t::plot(const string suffix)
{
    ...
    if(_nr==1) // only after r-average
    {
        ...
        {
            Zq_data.open(path_to_ens+"plots/Zq"+(suffix!=""?("_"+suffix):string(""))+"_m"+to_string(mass_val[m])+".txt");
            Zq_p2_data.open(path_to_ens+"plots/Zq"+(suffix!=""?("_"+suffix):string(""))+"_p2_m"+to_string(mass_val[m])+".txt");
            
            for(int imom=0; imom<in._linmoms; imom++)
            {
                Zq_data<<(in.p2_tilde)[imom]<<"\t"<<Zq_ave[imom][m]<<"\t"<<Zq_err[imom][m]<<endl;
    ...
    if(in._nr==1)
    {
        for(int m=0;m<in._nm;m++)
        {
            for(int ibil=0;ibil<nbil;ibil++)
            {
                Zbil_data[ibil].open(path_to_ens+"plots/Z"+bil[ibil]+(suffix!=""?("_"+suffix):string(""))+"_m"+to_string(mass_val[m])+".txt");
                Zbil_p2_data[ibil].open(path_to_ens+"plots/Z"+bil[ibil]+(suffix!=""?("_"+suffix):string(""))+"_p2_m"+to_string(mass_val[m])+".txt");
