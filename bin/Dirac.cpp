...
vprop_t make_gamma()
{
  ...
  for (int i_gam = 0; i_gam < 6; i_gam++)
    for (int i_row = 0; i_row < 4; i_row++)
      ...
      {
        gam[i_gam](isc(i_row, ic), isc(col[i_gam][i_row], ic)) =
            dcompl(real_part[i_gam][i_row], im_part[i_gam][i_row]);
      }
...
vprop_t create_projectors()
{
  ...
  vector<double> NL = {1.0, 4.0, 4.0, 4.0, 4.0, 1.0, 4.0, 4.0, 4.0,
                       4.0, 6.0, 6.0, 6.0, 6.0, 6.0, 6.0}; // normalize sum over Lorentz indices

  for (int igam = 0; igam < 16; igam++)
    P[igam] = GAMMA[igam].adjoint() / NL[igam];

  p_t p{0.442289, 0.5, 0.5, 0.5};
  double res = 0.0;
  prop_t lambda_igam;
  for (int ig = 1; ig <= 4; ig++)
  {
    lambda_igam = GAMMA[ig];

    res += (lambda_igam * GAMMA[ig]).trace().real() / 36.0;
    for (int ig2 = 1; ig2 <= 4; ig2++)
    {
      res += -0.25 * (lambda_igam * GAMMA[ig2]).trace().real() * p[ig2 % 4] / p[ig % 4] / 36.0;
    }
  }
  printf("res=%f\n", res);
  exit(0);
