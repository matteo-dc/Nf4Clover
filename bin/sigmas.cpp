      ...
      for (int i = 0; i < nins; i++)
        for (int ijack = 0; ijack < njacks; ijack++)
          for (int mr = 0; mr < nmr; mr++)
            sig[i][ijack][mr] =
                (p_slash * jprop_inv[i][ijack][mr]).trace().imag() / p2_tilde[imom] / (12. * V);
    }
    if (UseSigma1 == 1) // using "Sigma1" variant (eq.33 of 1004.1115)
    {
...
void oper_t::plot_sigmas()
{
  ...
  for (int j = 0; j < nsigma; j++)
  {
    ...
    for (int imom = 0; imom < _linmoms; imom++)
      sigma_data[j] << p2t[imom] << "\t" << sigma_ave[imom][iproj][ins][0] << "\t"
