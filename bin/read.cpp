...
void oper_t::read_mom_list(const string &path)
{
  ...
  while (!input.eof())
  {
    ...
    if (BC.compare("Antiperiodic") == 0)
      shift = {0.5, 0.0, 0.0, 0.0};

    for (int mu = 0; mu < 4; mu++)
    {
      input >> mom[mu];
      p_array[mu] = 2 * M_PI * (mom[mu] + shift[mu]) / sizeV[mu];
...
vvvd_t oper_t::read_eff_mass(const string name)
{
  ...
  for (int ijack = 0; ijack < njacks; ijack++)
    for (int m_fw = 0; m_fw < nm; m_fw++)
      for (int m_bw = 0; m_bw < nm; m_bw++)
      {
        ...
        if (rd != 1)
        {
          cerr << "Unable to read from \"" << name << "\" m_fw: " << m_fw << ", m_bw: " << m_bw
               << ", ijack: " << ijack << endl;
          exit(1);
        }
  ...
  for (int m_fw = 0; m_fw < nm; m_fw++)
    for (int m_bw = 0; m_bw < nm; m_bw++)
      printf("m1: %d \t m2: %d \t %lg +- %lg\n", m_fw, m_bw, eff_mass_ave[m_fw][m_bw],
             eff_mass_err[m_fw][m_bw]);
...
size_t isc(size_t is, size_t ic) { return ic + 3 * is; }
...
string path_to_conf(const string &string_path, const string &out, int i_conf, const string &name)
{
  ...
  {
    if (string_path.compare("/marconi_work/INF21_lqcd123/matteo/Nf4_Clover/B.d.145.24/") == 0)
      sprintf(path, "%s%s/%04d/fft_%s", string_path.c_str(), out.c_str(), i_conf, name.c_str());
    else
      sprintf(path, "%s%s/%04d/fft_%s_roman", string_path.c_str(), out.c_str(), i_conf,
              name.c_str());
  }
      ...
      for (int t = 0; t < ntypes; t++)
        for (int m = 0; m < nm; m++)
          for (int r = 0; r < nr; r++)
          {
            string hit_suffix = "";
            if (nhits > 1)
              hit_suffix = "_hit_" + to_string(ihit);

            int icombo = r + nr * m + nr * nm * t + nr * nm * ntypes * ihit +
                         nr * nm * ntypes * nhits * iconf;

            string path = path_to_conf(path_to_ens, out_hadr, conf_id[iconf],
                                       "S_" + Mass[m] + R[r] + Types[t] + hit_suffix);
    ...
    for (int ic_so = 0; ic_so < 3; ic_so++)
      for (int id_si = 0; id_si < 4; id_si++)
        for (int ic_si = 0; ic_si < 3; ic_si++)
        {
          ...
          if (rc != 1)
          {
            cerr << "Unable to read from " << path << " id_so: " << id_so << ", ic_so: " << ic_so
                 << ", id_si: " << id_si << ", ic_si:" << ic_si << endl;
            exit(1);
          }
...
vvvprop_t read_qprop_mom(FILE *input[], const vector<string> v_path, const int i_in_clust,
                         const int ihit, const int imom)
{
...
#pragma omp parallel for
  for (int ilin = 0; ilin < nm * nr * ntypes * njacks; ilin++)
  {
    ...
    int icombo =
        r + nr * m + nr * nm * t + nr * nm * ntypes * ihit + nr * nm * ntypes * nhits * iconf;
...
// read file
void read_internal(double &t, ifstream &infile) { infile >> t; }
void read_internal(VectorXd &V, ifstream &infile)
{
  for (int i = 0; i < V.size(); i++)
    read_internal(V(i), infile);
}
