...
vvd_t polyfit(const vvd_t &coord, const int n_par, vd_t &error, const vvd_t &y, const double xmin,
              const double xmax)
{
  ...
  for (int ijack = 0; ijack < njacks; ijack++)
  {
    ...
    for (int i = 0; i < count; i++)
      chi2[ijack] += (y_in_range[ijack][i] - poly[ijack][i]) *
                     (y_in_range[ijack][i] - poly[ijack][i]) /
                     (err_in_range[ijack][i] * err_in_range[ijack][i]);
...
vvd_t polyfit(const vvd_t &coord, const int n_par, vd_t &error, const vvd_t &y, const int range_min,
              const int range_max)
{
  MatrixXd S(n_par, n_par);
  vXd_t Sy(VectorXd(n_par), njacks);
...
vd_t fit_continuum(const vvd_t &coord, vd_t &error, const vd_t &y, const int p2_min,
                   const int p2_max, const double &p_min_value)
{
  ...
  for (int i = p2_min; i < p2_max; i++)
  {
    ...
    if (coord[1][i] > p_min_value)
    {
      ...
      for (int k = 0; k < n_par; k++)
        if (std::isnan(error[i]) == 0)
          Sy(k) += y[i] * coord[k][i] / (error[i] * error[i]);
    }
  }
  ...
  for (int ipar = 0; ipar < n_par; ipar++)
    jvpars[ipar] = jpars(ipar);

  return jvpars; // jpars[ibil][ijack][ipar]
}

vXd_t fit_chiral_jackknife(const vvd_t &coord, vd_t &error, const vector<vd_t> &y,
                           const int range_min, const int range_max, const double &p_min_value)
{
  int n_par = coord.size();
  ...
  for (int i = range_min; i < range_max; i++)
  {
    ...
    if (coord[1][i] > p_min_value)
    {
      ...
      for (int ijack = 0; ijack < njacks; ijack++)
        for (int k = 0; k < n_par; k++)
          if (std::isnan(error[i]) == 0)
            Sy[ijack](k) += y[i][ijack] * coord[k][i] / (error[i] * error[i]);
    }
  }
...
vvXd_t fit_chiral_jackknife(const vvd_t &coord, vvd_t &error, const vector<vvd_t> &y,
                            const int range_min, const int range_max, const double &p_min_value)
{
  int n_par = coord.size();
  ...
  for (int i = range_min; i < range_max; i++)
  {
    ...
    if (coord[1][i] > p_min_value)
    {
      for (int ibil = 0; ibil < nbil; ibil++)
        for (int j = 0; j < n_par; j++)
          for (int k = 0; k < n_par; k++)
            if (std::isnan(error[i][ibil]) == 0)
              S[ibil](j, k) += coord[j][i] * coord[k][i] / (error[i][ibil] * error[i][ibil]);
      ...
      for (int ibil = 0; ibil < nbil; ibil++)
        for (int ijack = 0; ijack < njacks; ijack++)
          for (int k = 0; k < n_par; k++)
            if (std::isnan(error[i][ibil]) == 0)
              Sy[ibil][ijack](k) +=
                  y[i][ibil][ijack] * coord[k][i] / (error[i][ibil] * error[i][ibil]);
    }
  }
...
vvd_t fit_par(const vvd_t &coord, const vd_t &error, const vvd_t &y, const int range_min,
              const int range_max /*,const string &path=NULL*/)
{
  int n_par = coord.size();
  int njacks = y.size();
